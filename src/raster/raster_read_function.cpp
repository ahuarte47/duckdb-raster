...
struct RT_Read {
		...
		for (idx_t col_idx = 0; col_idx < column_ids.size(); col_idx++) {
			...
			default: {
				...
				if (static_cast<int>(dim_index) < RASTER_FIRST_BAND_COLUMN_INDEX + num_bands) {
					...
					// TODO: Pack the raw binary data into the specified format before returning as BLOB.
					if (data_format != DataFormat::RAW) {
						throw NotImplementedException("Only RAW data format is supported for now");
					}
	...
	static void Execute(ClientContext &context, TableFunctionInput &input, DataChunk &output) {
		...
		// Loop over the tiles and fill the output chunk, it reuses a DataCube for avoiding repeated memory allocations.
