...
struct RT_ReadCells {
		...
		for (int32_t i = 0; i < count; i++) {
			...
			for (idx_t col_idx = 0; col_idx < column_ids.size(); col_idx++) {
				...
				case CELL_ROWID_COLUMN_INDEX:
					...
					break;
				case CELL_PIXEL_X_COLUMN_INDEX:
					output.data[col_idx].SetValue(row_idx, cell_x);
					break;
				case CELL_PIXEL_Y_COLUMN_INDEX:
					output.data[col_idx].SetValue(row_idx, cell_y);
					break;
				case CELL_COORD_X_COLUMN_INDEX:
					output.data[col_idx].SetValue(row_idx, cell_row.x);
					break;
				case CELL_COORD_Y_COLUMN_INDEX:
	...
	static constexpr auto DESCRIPTION = R"(
		...
		+ `pixel_x` is the pixel x coordinate of each cell in the raster, where 0 is the leftmost column.
		+ `pixel_y` is the pixel y coordinate of each cell in the raster, where 0 is the topmost row.
	...
	static constexpr auto EXAMPLE = R"(
		...
		SELECT * FROM RT_ReadCells('some/file/path/filename.tif');
	)";
