...
struct RT_Read {
	...
	static unique_ptr<FunctionData> BindUniqueFile(ClientContext &context, TableFunctionBindInput &input,
		...
		if (file_path.find("*") != std::string::npos) {
			...
			for (auto &file : files) {
				file_names.push_back(DuckDBDatasetFactory::GdalFilePath(context, file.path));
			}
		} else {
			file_names.push_back(DuckDBDatasetFactory::GdalFilePath(context, file_path));
		}
	...
	static unique_ptr<FunctionData> Bind(ClientContext &context, TableFunctionBindInput &input,
		...
		if (params.find("create_options") != params.end()) {
			dataset = GDALDatasetUniquePtr(DuckDBDatasetFactory::CreateDataset(context, file_names[0], params));
		} else if (params.find("warp_options") != params.end()) {
			dataset =
			    GDALDatasetUniquePtr(DuckDBDatasetFactory::WarpDataset(context, file_names, params, child_datasets));
		} else {
			dataset = GDALDatasetUniquePtr(DuckDBDatasetFactory::OpenDataset(context, file_names, params));
		}
